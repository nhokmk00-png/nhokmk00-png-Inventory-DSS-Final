...
| Database local      | SQLite                | Lưu dữ liệu khi chạy local                  |
...
Cách viết cẩn thận về cơ sở dữ liệu:

> Bản chạy local sử dụng SQLite để dễ cài đặt và trình bày. Về mặt thiết kế, hệ thống có chuẩn bị script SQL Server để phù hợp định hướng triển khai cơ sở dữ liệu quan hệ.

Không nên viết rằng hệ thống đã triển khai SQL Server hoàn chỉnh nếu nhóm chưa chạy thật trên SQL Server.
...
Không nộp các thư mục tự sinh: `node_modules`, `frontend/dist`, `.venv`, `__pycache__`.

---

### 16. Kịch bản trình bày hệ thống

1. Mở Trang chủ, giới thiệu báo cáo nhanh và trạng thái tồn kho.
2. Chỉ ra nhóm sản phẩm cần nhập và nhóm dư tồn/bán chậm.
3. Bấm vào một sản phẩm nguy cấp để xem gợi ý xử lý.
4. Bấm “Xem chi tiết sản phẩm” để mở tab Danh sách sản phẩm.
5. Giới thiệu giá nhập, giá bán, lãi gộp và biểu đồ nhu cầu.
6. Vào tab Xử lý đề xuất, duyệt hoặc điều chỉnh một đề xuất.
7. Gửi thông báo chi tiết qua Telegram hoặc lưu lịch sử thông báo nếu chưa cấu hình Telegram.
8. Vào tab Nhập hàng/Xuất hàng, lập phiếu có đơn giá thực tế.
9. Vào tab Nhập file Excel, upload file mẫu có đơn giá thực tế.
10. Tải báo cáo Excel và chỉ ra định dạng tiền `xxx.xxx.xxx`.

---

### 17. Gợi ý viết báo cáo theo chương

#### Chương 1 – Cơ sở lý thuyết

Nên viết khái niệm HTTTQL, thành phần HTTTQL, dữ liệu và thông tin, mô hình Input – Process – Output – Feedback, DSS, Dashboard/BI, quản lý tồn kho bán lẻ và vai trò hỗ trợ của AI/Gemini. Cần liên hệ với đề tài ở từng mục, không viết lý thuyết rời rạc.

#### Chương 2 – Thực trạng / yêu cầu bài toán

Nên viết các vấn đề: thiếu hàng, dư tồn, bán chậm, giá nhập thay đổi, giá xuất thay đổi theo khách hàng lớn hoặc khuyến mãi, dữ liệu Excel dễ sai, quản lý khó ra quyết định nếu chỉ nhìn bảng dữ liệu thô.

#### Chương 3 – Thiết kế và xây dựng hệ thống

Nên viết kiến trúc hệ thống, luồng dữ liệu, thiết kế dữ liệu, chức năng chính, logic xử lý tồn kho, logic giá và giao dịch, Dashboard, Gemini, Telegram và kịch bản trình bày.

Không nên viết Chương 3 như hướng dẫn code quá chi tiết. Nên viết theo hướng thiết kế HTTTQL và minh họa thực tế.

---

### 18. Những câu không nên viết trong báo cáo

Không nên viết:

```text
Hệ thống là ERP hoàn chỉnh.
Hệ thống tự động ra quyết định thay quản lý.
Gemini là mô hình dự báo chính.
Hệ thống dùng Deep Learning/LSTM/Transformer.
Hệ thống đã triển khai thật cho doanh nghiệp.
Hệ thống dùng dữ liệu thật của cửa hàng.
Dashboard chỉ để vẽ biểu đồ.
Chỉ có chức năng nhập hàng.
Không có xử lý dư tồn.
Không có xử lý giá giao dịch.
```

Nên viết:

```text
Hệ thống hỗ trợ nhà quản lý ra quyết định dựa trên dữ liệu tồn kho, nhu cầu bán, giá và giao dịch kho.
Hệ thống phân loại sản phẩm theo trạng thái quản lý như nguy cấp, cần nhập, dư tồn, bán chậm, theo dõi và an toàn.
Gemini hỗ trợ hỏi đáp và diễn giải kết quả quản trị.
Dữ liệu vận hành được chuẩn bị để minh họa đầy đủ các tình huống nghiệp vụ chính trong quản lý tồn kho bán lẻ.
```

---

### 19. Phân công nội dung cho nhóm

| Thành viên                  | Nội dung nên phụ trách                                                     |
| --------------------------- | -------------------------------------------------------------------------- |
| Thành viên phụ trách HTTTQL | Chương 3, kiến trúc hệ thống, luồng dữ liệu, Dashboard, nghiệp vụ hệ thống |
| Thành viên logistics 1      | Quản lý tồn kho, nhập/xuất hàng, tồn kho an toàn, điểm đặt hàng lại        |
| Thành viên logistics 2      | Dư tồn, bán chậm, điều chuyển, khuyến mãi, xử lý hàng lỗi                  |
| Thành viên logistics 3      | Giá nhập/xuất, giá trị vốn tồn, doanh thu, lãi gộp, báo cáo quản lý        |

Tất cả thành viên cần thống nhất thuật ngữ, không mỗi người gọi một kiểu.

---

### 20. Thuật ngữ nên dùng thống nhất
...
### 21. Checklist trước khi nộp

- Chạy được backend.
- Chạy được frontend.
- Trang chủ có dữ liệu.
- Biểu đồ có màu.
- Danh sách sản phẩm lọc được.
- Lập phiếu nhập/xuất có đơn giá thực tế.
- Upload Excel được.
- Tải báo cáo Excel được.
- Gemini trả lời được các câu hỏi khác nhau.
- Telegram gửi được nếu đã cấu hình, hoặc lưu lịch sử nếu chưa cấu hình.

---

### 22. Lưu ý cuối cho nhóm viết báo cáo
...
Các hướng phát triển có thể bổ sung trong tương lai:

- Đăng nhập và phân quyền người dùng.
- Tách vai trò nhân viên kho, quản lý kho và cấp quản lý.
- Quản lý nhiều kho.
- Quản lý nhiều chi nhánh.
- Điều chuyển hàng giữa các kho.
- Quét mã vạch hoặc QR sản phẩm.
- Phân tích tồn kho theo ABC/XYZ.
- Quản lý nhà cung cấp.
- Tự động tạo đơn đặt hàng từ đề xuất đã duyệt.
- Xuất báo cáo PDF.
...
- Thêm CI/CD.
- Thêm unit test cho backend.
- Thêm test giao diện frontend.
- Ghi log toàn bộ thao tác người dùng.
...
```text
nhokmk00-png/nhokmk00-png-Inventory-DSS-Final
```
...
Project hiện chưa khai báo giấy phép chính thức.

Nếu sử dụng để triển khai thực tế hoặc chia sẻ công khai, nên bổ sung file `LICENSE` phù hợp.

---

## Ghi chú sử dụng bản gộp

- Nếu cần đưa vào GitHub, có thể đổi tên file này thành `README.md`.
- Nếu gửi cho nhóm viết báo cáo, nên yêu cầu mọi người đọc kỹ **Phần 1** trước để thống nhất phạm vi đề tài.
- Khi cần chạy project hoặc kiểm tra API, xem **Phần 2**.
- Không tách đề tài sang ERP, logistics app, website bán hàng hoặc hệ thống AI tự động ra quyết định.
- Khi trình bày môn HTTTQL, luôn nhấn mạnh luồng: **Input → Process → Output → Feedback**.
